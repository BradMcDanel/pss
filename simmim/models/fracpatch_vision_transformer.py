...
def get_patch_idxs(x, drop_func, drop_ratio, predictor_lg=None):
    ...
    elif drop_func == "l2_magnitude":
        return get_l2_magnitude_patches(x, drop_ratio)
    elif drop_func == "gate":
...
def get_l2_magnitude_patches(x, drop_ratio):
    B, N, C = x.shape
    keep_point = int(round((1-drop_ratio)*N)) - 1

    # sample patches using their l2 magnitude
    sample_mat = (x[:, 1:]**2).sum(dim=2)
    sample_mat = sample_mat / torch.sum(sample_mat, dim=1, keepdim=True)
...
def get_gate_patches(x, drop_ratio, predictor_lg):
    ...
    return ((batch_idxs, patch_idxs), (B, keep_point + 1)), x
